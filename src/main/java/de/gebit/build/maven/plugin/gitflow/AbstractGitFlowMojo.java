...
public abstract class AbstractGitFlowMojo extends AbstractMojo {
	...
	/**
	 * A regex pattern that a new feature name must match. It is also used to extract a "key" from a branch name which
	 * can be referred to as <code>@key</code> in commit messages. The extraction will be performed using the first
	 * matching group (if present). You will need this if your commit messages need to refer to e.g. an issue tracker
	 * key.
	 *
	 * @since 1.3.0
	 */
	...
	/**
	 * When set to <code>true</code> before checking for local changes a `git status` will be performed. This way any
	 * non-real changes (CRLF) will be reconciled.
	 *
	 * @since 1.3.6
	 */
	...
	private boolean executeGitHasUncommitted() throws MojoFailureException, CommandLineException {
		...
		final CommandResult diffCommandResult =
				executeGitCommandExitCode("diff", "--no-ext-diff", "--ignore-submodules", "--quiet", "--exit-code");
		...
		if (diffCommandResult.getExitCode() == SUCCESS_EXIT_CODE) {
			...
			final CommandResult diffIndexCommandResult =
					executeGitCommandExitCode("diff-index", "--cached", "--quiet", "--ignore-submodules", "HEAD", "--");
	 ...
	 *
	 * @param branchName Branch name to checkout. You can give a commit id, too. In this case you will end up with a
	 *            detached HEAD.
	 * @throws MojoFailureException
	 * @throws CommandLineException
	 */
	...
	protected void gitRebaseFeatureCommit(final String featureIssue) throws MojoFailureException, CommandLineException {
		...
		try {
			...
			String tempFeatureVersion =
					versionInfo.getReleaseVersionString() + "-" + featureIssue + "-" + Artifact.SNAPSHOT_VERSION;
	...
	/**
	 * Returns the commit hash of the top-most commit in the current branch. If there is no commit, an exception is
	 * thrown.
	 *
	...
	/**
	 * Returns the commit message of the top-most commit in the current branch. If there is no commit, an exception is
	 * thrown.
	 *
	...
	/**
	 * Executes git symbolic-ref --short HEAD to get the current branch. Throws an exception when in detached HEAD
	 * state.
	 *
	 * @throws MojoFailureException
	 * @throws CommandLineException
	 ...
	 *
	 * @param tagPrefix Prefix of release tags.
	 * @throws MojoFailureException
	 * @throws CommandLineException
	 */
	 ...
	 *
	 * @param tagPrefix Prefix of release tags.
	 * @param pattern filter by shell pattern. Can be <code>null</code>.
	 * @throws MojoFailureException
	 * @throws CommandLineException
	 */
	 ...
	 * @param branchPoint the branch point on both feature and development branch
	 * @param versionChangeCommitId commit ID of the version change commit. Must be first commit on featuereBranch after
	 *            branchPoint
	 ...
	 * @return commit ID of the branch point (common ancestor with the development branch)
	 * @throws MojoFailureException if no branch point can be determined
	 */
	protected String gitFeatureBranchBaseBranch(String featureBranch)
		...
		for (Map.Entry<String, String> entry : branchPointCandidates.entrySet()) {
			...
			String revlistout =
					executeGitCommandReturn("rev-list", "--left-right", "--count", commit + "..." + featureBranch);
			...
			if (counts != null && counts.length > 1) {
				if (!"0".equals(org.apache.commons.lang3.StringUtils.deleteWhitespace(counts[1]))) {
					int branchDistance =
							Integer.parseInt(org.apache.commons.lang3.StringUtils.deleteWhitespace(counts[1]));
	 ...
	 *
	 * @param featureBranch feature branch name
	 * @param branchPoint commit ID of the common ancestor with the development branch
	 */
	protected String gitVersionChangeCommitOnBranch(String featureBranch, String branchPoint)
		...
		final String firstCommitOnBranch =
				(firstLineEnd == -1 ? gitOutput : gitOutput.substring(0, firstLineEnd).trim());
	...
	/**
	 * Execute git rev-list [branchPoint]..[branchName] --merges to check whether there are merge commits in the given
	 * feature branch from the given branch point. This is useful to determine if a rebase can be done.
	 *
	 * @param branchName The feature branch name.
	 * @param branchPoint commit id of the branching point of the feature branch from develop.
	 ...
	 *
	 * @param branchName Branch name to set a specific reference for
	 * @param newRef Value to set the reference to
	 * @throws MojoFailureException
	 * @throws CommandLineException
	 * @since 1.5.9
	 */
	...
	/**
	 * Checks if the given branch exists on the configure origin remote, and if so, executes git push [origin] --delete
	 * <branch_name>.
	 *
	 * @param branchName Branch name to delete.
	 * @throws MojoFailureException
	...
	protected void gitFetchRemoteAndCompare(final String branchName) throws MojoFailureException, CommandLineException {
		if (!gitFetchRemoteAndCompare(branchName, new Callable<Void>() {
			...
			getLog().warn("There were some problems fetching remote branch '"
	...
	protected boolean gitFetchRemoteAndCompare(final String branchName, Callable<Void> diffFunctor)
		...
		if (result.getExitCode() == SUCCESS_EXIT_CODE) {
			...
			getLog().debug("Comparing local branch '"
					+ branchName
					+ "' with remote '"
					+ gitFlowConfig.getOrigin()
					+ "/"
	 ...
	 * @param pushTags If <code>true</code> adds <code>--follow-tags</code> argument to the git <code>push</code>
	 *            command.
	 * @param force <code>true</code> to force non-FF (e.g. for feature branches)
	 * @throws MojoFailureException
	 * @throws CommandLineException
	 */
	 ...
	 *
	 * @param branchName name of the branch to check for.
	 ...
	 *
	 * @param commitId the commit to start the interactive rebase. Must be a predecessor of the current branch tip.
	 * @return <code>true</code> if the rebase is finished, <code>false</code> if further steps are needed
	 * @throws MojoFailureException
	 * @throws CommandLineException
	 */
	protected boolean gitRebaseInteractive(String commitId) throws MojoFailureException, CommandLineException {
		...
		cmdGit.clearArgs();
		cmdGit.addArguments(new String[] {
				"rebase", "--interactive", commitId
		});

		ProcessBuilder processBuilder =
				new ProcessBuilder(cmdGit.getShellCommandline()).directory(cmdGit.getWorkingDirectory()).inheritIO();
	...
	protected void gitBranchConfigWorktree(final String branchName, final String configBranchName,
		...
		FileBasedConfigurationBuilder<FileBasedConfiguration> builder =
				new FileBasedConfigurationBuilder<FileBasedConfiguration>(PropertiesConfiguration.class)
						.configure(params.properties().setThrowExceptionOnMissing(false));
		...
		CommandResult result =
				executeCommand(worktreeCmd, false, "commit", "-m", commitMessages.getBranchConfigMessage());
	 ...
	 *
	 * @param version New version to set.
	 * @param promptPrefix Specify a prompt prefix. A value <code>!= null</code> triggers processing of
	 *            {@link #additionalVersionCommands}
	 * @throws MojoFailureException
	 * @throws CommandLineException
	 */
	protected void mvnSetVersions(final String version, String promptPrefix)
		...
		if (tychoBuild) {
			...
			executeMvnCommand(false,
					VERSIONS_MAVEN_PLUGIN_SET_GOAL,
					"-DnewVersion=" + version,
	...
	/**
	 * Get the command specific additional commands to execute when a version changes.
	 *
	 ...
	 *
	 * @param copyOutput Copy output to console.
		...
		if (copyProperties != null) {
			for (String userProperty : copyProperties) {
				if (session.getRequest().getUserProperties().containsKey(userProperty)) {
					...
					newEffectiveArgs[effectiveArgs.length] = "-D"
							+ userProperty
							+ "="
							+ CommandLineUtils
	...
	/**
	 * Substitute keys of the form <code>@{name}</code> in the messages. By default knows about <code>key</code>, which
	 * is extracted from the feature branch name and all project properties.
	 *
	 * @param message the message to process.
	 * @param featureBranchName the branch name without prefix used to extract the key from.
	...
	/**
	 * Performs strings replacements in the given message, specified with the given replacement map. Placeholders in the
	 * message must be declared as
	 *
	 * @{someName} while the key in the map shall be just someName. If the map does not contain a found placeholder, it
	 *             will also look in the project properties.
	 * @param aMessage
	 ...
	 * @throws CommandLineException
	 * @throws MojoFailureException If <code>failOnError</code> is <code>true</code> and command exit code is NOT equals
	 *             to 0.
	 */
	...
	protected void checkGitWriteable() throws MojoFailureException, CommandLineException {
		String tempBranchName =
				"temp/" + System.getProperty("user.name").toLowerCase().replace(' ', '-') + "_access_check";
	...
	protected String makeValidTychoVersion(String version) throws VersionParseException {
		...
		if (versionInfo.getDigits().size() <= 4) {
			...
			result += "-"
					+ StringUtils.join(
							versionInfo.getDigits().subList(4, versionInfo.getDigits().size() - 1).iterator(), "_");
		}
		return result;
	}
