 ...
 */
public class VersionlessFileTest extends AbstractGitFlowMojoTestCase {
    private static final String PROMPT_NEW_VERSION = "What is the new version?";
    private static final String POM_NEXT_RELEASE_VERSION = TestProjects.VERSIONLESS_FILE_PROJECT.nextReleaseVersion;
    ...
    protected CoreExtension getCoreExtensionDescriptor() {
        ...
        desc.setGroupId(TestProjects.VERSIONLESS_FILE_PROJECT.extensionGroupId);
        desc.setArtifactId(TestProjects.VERSIONLESS_FILE_PROJECT.extensionArtifactId);
        desc.setVersion(TestProjects.VERSIONLESS_FILE_PROJECT.extensionVersion);
        return desc;
    }

    @Before
    public void setUp() throws Exception {
        repositorySet = git.useGitRepositorySet(TestProjects.VERSIONLESS_FILE_PROJECT);
    }
    ...
    public void testExecuteInInteractiveMode() throws Exception {
        ...
        git.assertModifiedFiles(repositorySet, AbstractGitFlowMojo.POM_VERSION_FILE);
        assertVersionsInPom(repositorySet.getWorkingDirectory(), "${revision}");
        assertEquals(NEW_VERSION,
                FileUtils.readFileToString(
                        new File(repositorySet.getWorkingDirectory(), AbstractGitFlowMojo.POM_VERSION_FILE),
