...
public class GitFlowEpicAbortMojoTest extends AbstractGitFlowMojoTestCase {
    ...
    public void testExecuteOnEpicBranchTwoEpicBranchesAndOtherBranch() throws Exception {
        ...
        final String OTHER_BRANCH = "otherBranch";
        git.createBranchWithoutSwitch(repositorySet, OTHER_BRANCH);
        ExecutorHelper.executeEpicStart(this, repositorySet, EPIC_NAME_2);
        git.switchToBranch(repositorySet, MASTER_BRANCH);
        ExecutorHelper.executeEpicStart(this, repositorySet, EPIC_NAME);
        git.createAndCommitTestfile(repositorySet);
        // test
    ...
    @Test
    public void testExecuteWithSpecialEpicBranchPrefix() throws Exception {
        // set up
        final String OTHER_BRANCH = "epic/" + EPIC_NAME;
        final String EPIC_BRANCH_PREFIX = "epics/PREFIX-";
        final String EPIC_BRANCH1 = EPIC_BRANCH_PREFIX + EPIC_NAME;
        final String EPIC_BRANCH2 = EPIC_BRANCH_PREFIX + EPIC_NAME_2;
        Properties userProperties = new Properties();
        userProperties.setProperty("flow.epicBranchPrefix", EPIC_BRANCH_PREFIX);
        git.createBranchWithoutSwitch(repositorySet, OTHER_BRANCH);
        ExecutorHelper.executeEpicStart(this, repositorySet, EPIC_NAME, userProperties);
        git.createAndCommitTestfile(repositorySet);
        git.switchToBranch(repositorySet, MASTER_BRANCH);
        ExecutorHelper.executeEpicStart(this, repositorySet, EPIC_NAME_2, userProperties);
        git.switchToBranch(repositorySet, MASTER_BRANCH);
        String PROMPT_MESSAGE = "Epic branches:" + LS + "1. " + EPIC_BRANCH1 + LS + "2. " + EPIC_BRANCH2 + LS
                + "Choose epic branch to abort";
        when(promptControllerMock.prompt(PROMPT_MESSAGE, Arrays.asList("1", "2"))).thenReturn("1");
        // test
        executeMojo(repositorySet.getWorkingDirectory(), GOAL, userProperties, promptControllerMock);
        ...
        git.assertClean(repositorySet);
        git.assertCurrentBranch(repositorySet, MASTER_BRANCH);
        git.assertLocalBranches(repositorySet, MASTER_BRANCH, EPIC_BRANCH2, OTHER_BRANCH, CONFIG_BRANCH);
        git.assertRemoteBranches(repositorySet, MASTER_BRANCH, EPIC_BRANCH2, CONFIG_BRANCH);
        git.assertLocalAndRemoteBranchesAreIdentical(repositorySet, MASTER_BRANCH, MASTER_BRANCH);
        git.assertCommitsInLocalBranch(repositorySet, MASTER_BRANCH);
        assertVersionsInPom(repositorySet.getWorkingDirectory(), TestProjects.BASIC.version);
    }
